...
GAME_ITER = 50
RESULT_DISPLAY_TIME = 0.1

log_directory = "accuracy_logs/"
p1_name = players[1](board=board, available_pieces=available_pieces).name
p2_name = players[2](board=board, available_pieces=available_pieces).name
log_file_name = f"p1_{p1_name}_p2_{p2_name}_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

open(log_directory + log_file_name, "w")

while game_count < GAME_ITER:
    turn = 1 
    flag = "select_piece"
    game_over = False
    selected_piece = None
    ...
    with open(log_directory + log_file_name, "a") as f:
        f.write(f"date {datetime.datetime.now()}, game_count: {game_count}\n")
        f.write(f"p1_name: {players[1](board=board, available_pieces=available_pieces).name}, p2_name: {players[2](board=board, available_pieces=available_pieces).name}\n")
        f.write(f"p1_total_time_consumption: {total_time_consumption[1]}, p2_total_time_consumption: {total_time_consumption[2]}\n")
        f.write(f"winner: {winner}\n")
        f.write(f"board: {board}\n")
        f.write(f"player_1_win_rate: {player_1_win_count / game_count}\n")
        f.write(f"player_2_win_rate: {player_2_win_count / game_count}\n")
        f.write(f"-----------------------------------\n")
